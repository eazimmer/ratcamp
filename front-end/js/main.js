    ...
    if (data.code == 'start') {
      ...
      countdownTimer = setInterval(() => {
        if(countdownTimerValue <= 0){ // run out of time
          ...
          /*
          // select correct answer and disable click/hover
          let element = $('.trivia-question')[$('.trivia-question').length-1];
          element.children('.trivia-correct-answer .trivia-answer-btn').css('background', '#4b7a41');
          element.children('.trivia-correct-answer .trivia-answer-txt').css('color', '#4b7a41');

          element.children('li').unbind('click');
          element.children('li').css('pointer-events', 'none');
          */
...
const questionAnswered = (e) => {
  if ($(e).hasClass('trivia-correct-answer')) {
    ...
    sendAnswer(urlParams.get('name'), countdownTimerValue);
