...
function signup() {
  ...
  var credentials_object = {'email': email, 'name': name, 'password': password}

  credentials_object['password'] =
      encrypt_and_decrypt(credentials_object['password'], true)
...
function login() {
  ...
  var credentials_object = {'name': name, 'password': password}
...
$(document).ready(function() {
  ...
  socket.on('signup-result', msg => {
    if (msg === 'Success') {
      ...
      document.getElementById('result').innerHTML =
          'Signup attempt failed, an account already exists using this username. Please try again and provide a different username.'
    }
  });
  ...
  socket.on('login-result', msg => {
    if (msg === 'Success') {
      ...
      socket.emit(
          'already-online-check', document.getElementById('name-input').value)
    } else {
      document.getElementById('result').innerHTML =
          'Login attempt failed, invalid credentials. Please try again.'
    }
  });

  // Handle response to whether or not account is already
  // logged on
  socket.on('online-check-result', found => {
    if (!found) {  // Not already logged in; login
      ...
      const url = './messageBoard.html?name=' +
          document.getElementById('name-input').value;
      window.location.replace(url);
    } else {  // Account already logged in; abort
      document.getElementById('result').innerHTML =
          'Login attempt failed, this account is already signed in. Please try a different account.'
    }
  });
...
const sendMessage =
    () => {
      ...
      if ($.trim(message).length !== 0) {
        ...
        socket.emit('chat', JSON.stringify(msgData));
      }
    }
...
socket.on('trivia-update', data => {
  if (data.code == 'start') {
    ...
    outputQuestion(
        data.questionNum, data.question, data.answers, data.correct_index);
  }
});
...
const sendAnswer = (username, points) => {
